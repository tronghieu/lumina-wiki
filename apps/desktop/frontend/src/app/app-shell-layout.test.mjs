...
test('app shell follows the hand-drawn three-zone layout contract', () => {
  ...
  assert.match(inspectorSource, /className="agent-composer"/);
});
...
test('layout css defines desktop zones and compact fallbacks', () => {
  ...
  assert.match(cssSource, /\.graph-menu-settings\s*{[^}]*grid-row:\s*5/s);
...
test('unimplemented chat controls are visibly unavailable', () => {
  assert.match(inspectorSource, /className="new-chat-button"[^>]*disabled/);
  assert.match(inspectorSource, /aria-label="Chat input"[^>]*disabled/);
  assert.match(inspectorSource, /type="button" disabled>Send</);
});
