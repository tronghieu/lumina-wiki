...
- [认识 /lumi-help —— 你的智能向导](#认识-lumi-help--你的智能向导)
...
`/lumi-help` 是所有命令里最值得先记住的一条。

忘了刚才在干嘛？敲 `/lumi-help`。打开项目，不知道从哪儿下手？敲 `/lumi-help`。想看看 Lumina-Wiki 全部能做什么？`/lumi-help skills`。好奇 Lumina 在底层 *怎么工作*？`/lumi-help explain <主题>`。

```text
/lumi-help                       # 基于你 wiki 当前情况的一条下一步建议
/lumi-help skills                # 你已安装的全部命令清单
/lumi-help explain <主题>         # 询问 Lumina 自己的工作原理（例如 bidirectional links）
```

可以把它想成一位已经熟悉你项目的向导：

- 它会瞄一眼 `raw/` 和 `wiki/`，看看你上次做到了哪一步。
- 它挑出最合理的下一步——通常是一条命令，附上可以直接复制的命令行。
- 加上 `skills`，它会把你装的所有命令按 pack（Core / Research / Reading）一一列出来。
- 加上 `explain <主题>`，它会回答关于 Lumina 本身的问题——`raw/` 和 `wiki/` 有什么不同、为什么链接必须双向、`lumi-check` 到底检查什么——并附上本地文档引用以便你核对。

本指南剩下的部分讲 Lumina-Wiki *是什么*、*能做什么*。装好之后，你几乎不用再记 *怎么用*——那是 `/lumi-help` 的活儿。
...
### 4. 卡住了？敲 `/lumi-help`
忘了自己在干嘛？想看看都有哪些命令？需要回想一下 Lumina 的某个概念？敲一下：
```text
/lumi-help                       # 一条下一步建议
/lumi-help skills                # 你已安装的所有命令
/lumi-help explain <主题>         # 询问 Lumina 自己的工作原理
```
默认模式会看一眼你的 wiki，然后告诉你下一步该做什么——刚装完那一刻是这个答案，半年后再打开还是这个答案。加 `explain <主题>` 切换到关于 Lumina 本身的简短问答，并附上本地文档引用。
...
| `/lumi-help` | 让 AI 根据你 wiki 的当前情况给一条下一步建议。加 `skills` 可看到全部命令，或 `explain <主题>` 询问 Lumina 自己的工作原理。 |
...
`/lumi-help` 会看一眼你目前的 wiki，告诉你接下来该做的一件事——当你忘了上次停在哪里、或者不确定该用哪个命令时很有用。如果想看 Lumina-Wiki 能做的全部事情，加 `skills`。如果想知道 Lumina 自己 *怎么工作*——`raw/` 和 `wiki/` 各自做什么、为什么链接必须双向、什么是 "foundation" 页面——加 `explain <主题>`。

### 什么时候用

- 隔了一段时间再打开项目，不确定从哪里开始。
- 往 `raw/` 里放了几份文件，但忘了下一步要做什么。
- 想看一下你这次安装里有哪些命令可用（用 `skills`）。
- 想要一个简短、有引用的回答来回顾 Lumina 的某个概念（用 `explain <主题>`）。

### 怎么用

```text
/lumi-help                       # 给出一条下一步建议
/lumi-help skills                # 列出你已安装的所有命令
/lumi-help explain <主题>         # 关于 Lumina 自己的简短问答
```

### 你会得到什么

默认模式：一条建议命令、一段简短理由（基于 wiki 当前的真实情况，例如"`raw/` 里还有三份文件没纳入"），以及可以直接复制运行的命令行。如果 wiki 超过一个月没有更新，会附上一行提示，建议你之后跑一次 `/lumi-check`——但主要建议依然是继续推进工作，而不是先做核查。

`skills` 模式：按 pack（Core / Research / Reading）分组列出每一个命令，每条命令一行说明。只有你实际安装了的 pack 部分才会显示。
