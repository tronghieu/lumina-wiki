...
如果你同时维护不止一个 wiki（比如一个用于工作阅读，一个用于个人项目），或者你希望直接在聊天里联系你的 wiki，而不是每次都要打开代码编辑器，就可以用这个方式。
...
1. 先为你使用的每个助手全局安装一次辅助功能：
    ...
    （如果你用的是 Hermes，把 `openclaw` 换成 `hermes`）。这一步只是让助手知道去哪里找你的 wiki —— 每个 wiki 本身仍然需要按本指南前面的步骤正常安装。

2. 告诉它你已有的每个 wiki：

    ```bash
    lumina wikis add /你的/wiki/路径 --name "AI 工程" --alias "AI"
    ```

    给它起一个清楚的名字，如果愿意也可以加一个简短的别名 —— 用你聊天时会自然说出的词。
