...
- [安装](#安装)
- [Lumina Desktop 预览版](#lumina-desktop-预览版)
...
Lumina Desktop 是面向桌面端测试参与者的尚未发布纯应用预览版，目前还没有公开下载。如果你正在从本代码仓库开发或评估它，请按照[桌面端 README](../../apps/desktop/README.md) 中的设置说明操作。

首次启动时，欢迎页面让你创建第一个本机知识库，或打开已有的 Lumina 知识库。“创建”会准备并打开一个标准知识库；应用运行时不需要 Node.js、npm、Python、Lumina CLI 或命令行。“打开”会先检查所选知识库，而且不会改写其中的文件。

应用会把最近使用的知识库记录在自己的私有本机数据中。再次打开时，它可以按知识库恢复最近保存的对话、你正在阅读的笔记，以及“聊天”“笔记”或“图谱”视图。如果最近的知识库已被移动、删除或替换，应用会停止并回到安全恢复路径，不会把另一个文件夹当成原来的知识库。对话历史为可选功能，可在应用中关闭或清除。

当前预览版主要用于创建或打开知识库、阅读笔记和关系，并继续当前知识库中的对话。“检查”“导入”、更新、修复、重置及其他维护流程尚未进入当前纯应用界面，暂时请使用 CLI 完成这些任务。macOS、Windows 和 Linux 安装版的人工验收，以及应用签名和可信分发，也仍待完成。
...
| `/lumi-ask <问题>` | 向 `wiki/` 中的知识库提问。它总会说明答案依据了哪些页面，如果缺少信息，还会指出 `raw/sources/` 中可能相关的文件。 |
...
当资料非常长时 — 一整本书、一篇学位论文，或大约 50 页以上的任何文档 — AI 会改变阅读方式。它不再一次略读后直接总结，而是先梳理全书结构，再逐章阅读，为每一部分写一份笔记，并在每个要点旁标注页码。总结页面随后从这些笔记写成，每一句引文都会与原书页面核对，确保没有凭空编造的内容。

你会照常得到总结页面，另外还有 `wiki/readings/` 中的一个阅读笔记文件夹 — 每章一份笔记。之后关于这本书的问题都从这些笔记中回答，AI 不必重读整本书，你也总能把答案追溯到确切的页码。如果阅读中途被打断，再次运行 `/lumi-ingest` 会从最近读完的一章继续。

需要注意：如果你自己正在读一本小说、不想被剧透结局，就不要用这种方式添加整本书 — 请改用 Reading Pack 的逐章命令，它只会知道你已经读到的部分。
...
因为新的原始资料还只是放在 `raw/` 里。如果你在放入 wiki 之前就问起它，`/lumi-ask` 会告诉你 wiki 还没有这部分内容，并指出 `raw/sources/` 里哪些文件可能有答案——这样你可以直接打开阅读。要让 AI 也能用上它，请先用下面的命令把它放入 wiki：
