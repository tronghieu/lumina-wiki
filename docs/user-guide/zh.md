...
忘了刚才在干嘛？敲 `/lumi-help`。打开项目，不知道从哪儿下手？敲 `/lumi-help`。想看看 Lumina-Wiki 全部能做什么？`/lumi-help skills`。
...
### 4. 卡住了？敲 `/lumi-help`
忘了自己在干嘛？想看看都有哪些命令？敲一下：
```text
/lumi-help            # 一条下一步建议
/lumi-help skills     # 你已安装的所有命令
```
它会看一眼你的 wiki，然后告诉你下一步该做什么。刚装完那一刻是这个答案，半年后再打开还是这个答案。
...
| `/lumi-help` | 让 AI 根据你 wiki 的当前情况给一条下一步建议。加 `skills` 可看到全部命令。 |
...
`/lumi-help` 会看一眼你目前的 wiki，告诉你接下来该做的一件事——当你忘了上次停在哪里、或者不确定该用哪个命令时很有用。如果想看 Lumina-Wiki 能做的全部事情，在命令后加 `skills` 就能看到完整清单。
