...
`/lumi-ingest` 读取一份原始资料（PDF、文章、网页等），然后在 `wiki/` 中为它写一个总结页面。与以前不同，现在它会在四个检查点暂停，让你在内容正式保存之前亲自看一眼。
...
1. **草稿** — AI 阅读来源，写出 wiki 页面的初稿。暂停，让你查看草稿。
2. **结构检查** — AI 检查链接是否完整、相关页面是否都已连接。暂停，让你查看结果。
3. **核查** — AI 把草稿里的陈述逐一和来源对比，标出有疑问的地方。暂停，让你审阅可疑之处。
4. **定稿** — AI 记录日志并保存页面。这一步不暂停，只是登记完成。

在每个检查点，你可以选择接受（继续下一步）、修改（调整后重新检查），或退出（暂停，稍后继续）。在核查检查点，如果有少量存疑但你仍想保存，还可以选择"带警告接受"，页面会被标记为可信度偏低。中途退出不会丢失进度——再次对同一份资料运行 `/lumi-ingest`，会从上次停下的检查点继续。

### 你会得到什么

- `wiki/sources/` 中一个新的总结页面，包含这份资料的要点。
- 与 wiki 中已有概念、人物的链接（让这份资料融入你的知识网络）。
- 自动新建的概念或人物页面，如果资料里出现了 wiki 中尚未记录的概念或作者。
- 日志记录：这份资料何时放入 wiki、核查结果如何。
- 如果核查发现问题并选择带警告接受，页面会带有可信度标注，便于以后再次审阅。
