...
<p align="center">
  <img src="../../assets/zalo-community-qr.jpg" alt="QR tham gia nhóm Zalo Lumina Wiki Early Adopters" width="280">
</p>
...
- [Dùng Wiki Của Bạn Từ Agent Chat (OpenClaw Hoặc Hermes)](#dùng-wiki-của-bạn-từ-agent-chat-openclaw-hoặc-hermes)
...
1.  **Ngôn ngữ trình cài đặt / Installer language / 语言** *(mới từ v1.2)*: Câu hỏi đầu tiên. Hãy chọn ngôn ngữ bạn muốn trình cài đặt dùng để nói chuyện với bạn — **English**, **Tiếng Việt**, hoặc **中文**. Đây chỉ là ngôn ngữ của phần thiết lập; sau khi cài xong, bạn vẫn có thể yêu cầu AI giao tiếp bằng bất kỳ ngôn ngữ nào. Nếu bạn cài lại và đổi ngôn ngữ, trình cài đặt sẽ hỏi xác nhận một lần trước khi đổi để tránh ghi đè nhầm.
2.  **Thư mục cài đặt (Installation directory)**: Nhấn **Enter** để xác nhận cài đặt trong thư mục hiện tại. Đây là lựa chọn được khuyến nghị.
3.  **Mục đích sử dụng (Research purpose)**: Mô tả ngắn gọn chủ đề bạn quan tâm (ví dụ: "Nghiên cứu về AI" hoặc "Ghi chú sách marketing"). Điều này giúp AI hiểu ngữ cảnh làm việc của bạn.
4.  **Công cụ AI (IDE targets)**: Chọn các công cụ chat AI bạn đang dùng. **OpenAI CodexApp (ChatGPT)** là một lựa chọn rất thuận tiện cho người mới vì nó có giao diện ứng dụng riêng. Ngoài ra bạn có thể chọn các công cụ mạnh mẽ khác như Gemini CLI, Claude Code, v.v. Dùng phím **Mũi tên** để di chuyển và phím **Cách (Space)** để chọn.
5.  **Gói tính năng (Packs)**: Chọn các kỹ năng bổ sung như `research` (cho nghiên cứu khoa học) hoặc `reading` (cho đọc sách). Các tính năng cốt lõi đã được bao gồm sẵn và không cần chọn.
6.  **Ngôn ngữ (Languages)**: Nhập "Tiếng Việt" để AI trao đổi và tạo ghi chú bằng tiếng Việt. (Mặc định sẽ khớp với ngôn ngữ trình cài đặt bạn vừa chọn ở bước 1, nhưng bạn có thể nhập ngôn ngữ khác nếu muốn.)
...
Cài đặt xong, điều đầu tiên nên thử là gõ `/lumi-help`. Đây là lệnh bạn dùng bất cứ lúc nào không biết làm gì tiếp theo.

```text
/lumi-help                      # gợi ý một bước tiếp theo phù hợp với dự án của bạn
/lumi-help skills               # xem danh sách tất cả các lệnh bạn có
/lumi-help explain <câu hỏi>    # hỏi về cách Lumina hoạt động
```

Hãy nghĩ về `/lumi-help` như một người bạn quen dự án của bạn:

- **Không kèm gì thêm:** AI nhìn vào dự án của bạn và đề xuất đúng một việc nên làm tiếp. Ví dụ: "Bạn có 2 tài liệu mới chưa nạp, hãy thử `/lumi-ingest`." Dùng khi không nhớ mình đang làm đến đâu, hoặc vừa cài xong và chưa biết bắt đầu từ đâu.
- **Thêm `skills`:** hiện danh sách đầy đủ các lệnh bạn đang có, mỗi lệnh kèm một câu giải thích ngắn.
- **Thêm `explain <câu hỏi>`:** giải thích nhanh về cách Lumina hoạt động. Ví dụ: `/lumi-help explain thư mục raw để làm gì` hay `/lumi-help explain lumi-check kiểm tra gì`.
...
### 4. Không biết làm gì tiếp? Gõ `/lumi-help`
Quên mất mình đang làm đến đâu? Vừa thêm tài liệu và không biết bước tiếp theo? Muốn xem tất cả các lệnh có sẵn? Cứ gõ:

```text
/lumi-help                      # gợi ý một bước tiếp theo
/lumi-help skills               # xem tất cả các lệnh bạn có
/lumi-help explain <câu hỏi>    # hỏi về cách Lumina hoạt động
```

AI nhìn vào dự án của bạn và đề xuất đúng một việc nên làm tiếp — ví dụ "Bạn có 3 tài liệu mới chưa nạp, hãy thử `/lumi-ingest`." Câu trả lời phụ thuộc vào tình trạng thực tế của dự án lúc bạn gõ.
...
Research Pack có bảy lệnh chính:
...
| `/lumi-research-watch-run` | Chạy một lượt khám phá theo lịch trên watchlist (chủ đề + nguồn RSS / Atom). Là phiên bản trong chat của `lumina discover run`; chỉ chạy khi bạn yêu cầu. |
...
hoặc GitHub Actions gọi, không phải do AI tự thức dậy. Nếu muốn chạy thử
một lượt ngay trong chat, dùng `/lumi-research-watch-run` — nó sẽ duyệt
qua mọi chủ đề và nguồn RSS trong watchlist một lần và báo cáo những gì mới.
Xem [Nâng cao: Tìm tài liệu định kỳ](advanced-scheduled-discovery.vi.md) để có ví dụ
...
| `/lumi-help` | Không biết làm gì tiếp? Gõ lệnh này để được gợi ý bước kế tiếp. Thêm `skills` để xem danh sách tất cả lệnh, hoặc `explain <câu hỏi>` để hỏi về cách Lumina hoạt động. |
...
## Khi Bạn Không Biết Làm Gì Tiếp — /lumi-help

`/lumi-help` là lệnh bạn có thể gõ bất cứ lúc nào: vừa cài xong chưa biết bắt đầu từ đâu, mở lại dự án sau vài ngày quên mất đang làm đến đâu, hoặc muốn biết Lumina-Wiki còn làm được gì khác.

### Khi nào nên dùng

- Bạn mở lại dự án sau một thời gian và không nhớ mình đang làm đến đâu.
- Bạn vừa thêm tài liệu mới và không chắc bước tiếp theo là gì.
- Bạn muốn xem tất cả những lệnh mình đang có.
- Bạn có thắc mắc về cách Lumina hoạt động và muốn giải thích nhanh.

### Cách dùng

```text
/lumi-help                      # gợi ý một bước tiếp theo
/lumi-help skills               # xem toàn bộ các lệnh đang có
/lumi-help explain <câu hỏi>    # hỏi về cách Lumina hoạt động
```

### Bạn nhận được gì

**Gõ `/lumi-help` không kèm gì:** AI nhìn vào dự án của bạn và đề xuất đúng một việc nên làm tiếp — ví dụ: "Bạn có 3 tài liệu mới chưa nạp vào wiki, hãy thử `/lumi-ingest`." Nếu bạn chưa đụng vào dự án hơn một tháng, nó sẽ thêm một gợi ý nhỏ để kiểm tra lại tình trạng trước khi tiếp tục.

**Gõ `/lumi-help skills`:** bạn thấy danh sách tất cả các lệnh chia thành từng nhóm, mỗi lệnh kèm một câu mô tả ngắn. Nhóm nào bạn chưa cài thì không hiện ra.

**Gõ `/lumi-help explain <câu hỏi>`:** AI trả lời ngắn gọn về cách Lumina hoạt động. Ví dụ: `/lumi-help explain thư mục raw để làm gì` hay `/lumi-help explain lumi-check kiểm tra gì`. Lưu ý: lệnh này chỉ giải thích về cách Lumina vận hành, không đọc nội dung wiki của bạn. Nếu muốn hỏi về nội dung wiki ("wiki của tôi nói gì về chủ đề X?"), hãy dùng `/lumi-ask`.
...
## Dùng Wiki Của Bạn Từ Agent Chat (OpenClaw Hoặc Hermes)

Nếu bạn dùng một agent chat luôn hoạt động như OpenClaw hoặc Hermes — có thể nhắn qua Telegram hay Lark — agent đó có thể chăm sóc nhiều wiki của bạn cùng lúc. Bạn gửi cho agent một tài liệu hoặc một câu hỏi từ bất cứ đâu bạn vẫn hay nhắn tin, và agent sẽ đưa tài liệu vào đúng wiki hoặc trả lời từ đúng wiki mà bạn không cần mở project nào cả.

Để xem hướng dẫn cài đặt và tích hợp đầy đủ, xem [Cài đặt và tích hợp
Lumina-Wiki với OpenClaw và Hermes](openclaw-hermes-integration.vi.md).
...
1. Cài đặt phần hỗ trợ cho agent của bạn, một lần cho mỗi agent bạn dùng:
    ...
    (đổi `openclaw` thành `hermes` nếu bạn dùng Hermes). Bước này giúp agent biết cách tìm và quản lý các wiki của bạn — bạn không cần cài đặt riêng cho từng wiki nữa, agent có thể tự thiết lập giúp bạn ngay trong lúc nhắn tin.
...
2. Cho nó biết về một wiki, ngay trong lúc nhắn tin — một wiki bạn đã có sẵn, hoặc một wiki hoàn toàn mới bạn muốn nó tạo ra:
    ...
    Đặt cho wiki một tên rõ ràng và, nếu muốn, một tên gọi tắt — chọn từ ngữ bạn sẽ tự nhiên nói ra khi nhắn tin. Agent luôn xem qua thư mục trước khi làm bất cứ điều gì: nếu đó đã là một wiki, agent chỉ đăng ký lại; nếu thư mục đã có sẵn file của bạn, agent sẽ cho bạn biết trong đó có gì và chờ bạn đồng ý trước khi thêm wiki vào; nếu thư mục còn trống, agent sẽ hỏi wiki này dùng để làm gì rồi thiết lập luôn trong một bước.

3. Sau đó chỉ cần nhắn tin bình thường. Gọi tên wiki khi bạn nhờ việc gì đó ("thêm file PDF này vào wiki kỹ thuật AI", "wiki Nấu Ăn nói gì về cách giữ dao?"), và agent sẽ hỏi lại nếu chưa chắc bạn muốn nói đến wiki nào.

### Bạn nhận được gì

Tài liệu bạn gửi sẽ được đưa vào đúng wiki và sắp xếp gọn gàng, giống như khi bạn tự thêm vào vậy. Bất cứ lúc nào bạn cũng có thể hỏi "tôi có những wiki nào?" hoặc "mọi thứ có ổn không?", và agent sẽ kiểm tra rồi báo lại bằng lời lẽ dễ hiểu. Việc kiểm tra sức khỏe này cũng có thể được đặt chạy theo lịch, để các vấn đề nhỏ được phát hiện và xử lý sớm trước khi chồng chất.

Một wiki được agent thiết lập theo cách này sẽ gọn nhẹ có chủ đích — chỉ có phần wiki mà thôi, vì các kỹ năng của agent đã sẵn có trên nền tảng chat của bạn rồi, không cần đặt thêm trong thư mục wiki. Nếu sau này bạn muốn mở đúng wiki đó trong một trình soạn thảo mã (Claude Code, Cursor, v.v.), hãy chạy lệnh cài đặt thông thường ở phần trước của hướng dẫn này ngay trong thư mục đó — đây là một bước riêng, tùy chọn, khi nào bạn cần thì làm.
