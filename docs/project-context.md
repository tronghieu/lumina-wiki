...
21. **User-facing docs target non-technical readers.** READMEs and `docs/user-guide/*.md` are written for end users without a technical or AI background. Lumina-Wiki's default audience is a person who wants a useful knowledge assistant, not someone who wants to learn the implementation. Product decisions, feature names, skills, guides, and visible messages should optimize for that audience first; technical details belong in advanced guides, development docs, or hidden implementation notes. Apply these rules whenever editing those files (and any other user-facing surface — installer output, skill descriptions in skill tables, error messages a user sees):
    ...
    - **Avoid jargon and internal concepts.** Do NOT use these words in user-facing prose: "hallucination", "anchoring", "frontmatter", "state file", "schema", "JSON", "subagent", "adversarial", "stage", "tier", "provenance", "drift", "reviewer", "context window", "prompt", "token". Translate to plain language: "AI making things up" → "checking that AI didn't add things not in your sources"; "frontmatter writeback" → "attached to the note"; "state file" → omit or call it "report".
    - **Hide implementation paths.** Do not mention code paths (`src/...`), config files (`_lumina/...`), specific frontmatter field names, or output JSON shapes in user-guide prose. If a user could not change it productively, they don't need to see it.
    - **Match the existing prose tone.** Existing user guides use friendly second person ("you read", "you may"), short sentences, plain examples. New sections must match.
    - **Respect configured language.** Installed agents must communicate entirely in the configured `communication_language` and write wiki pages in `document_output_language`. Do not mix English into Vietnamese/Chinese user-facing replies except for commands, file names, proper nouns, or quoted source text. Translate workflow/tool terms into the user's language; for important domain terms from a source, use the translated phrase first and put the source term in parentheses on first use.
    - **Ask only for human judgment.** User-facing workflows should not pause after every internal tool step. Ask when the user must approve content, resolve ambiguity, allow overwrite/restart, accept lower confidence, handle source-check findings, or decide how to fix an issue that tools cannot fix safely.
    - **Default to a four-part structure for new skill sections in user-guide:** (1) one-paragraph purpose in plain language, (2) when to use it, (3) how to invoke it (one or two example commands, no flag tables unless the user genuinely needs to choose), (4) what the user gets back.
    - This rule applies to user-guide updates, README skill-table descriptions, installer messages, skill `description:` frontmatter (which appears in IDE skill pickers), and any human-readable error message. It does NOT apply to: `docs/project-context.md`, `docs/planning-artifacts/*`, `docs/implementation-artifacts/*`, `ROADMAP.md`, `SKILL.md` body sections (those are for the agent, not the user), or any `*.test.*` file.
