...
- **`.agents/`** contains **only** `skills/`, laid out flat: one directory per skill named `lumi-<name>/` (e.g. `lumi-init/`, `lumi-discover/`, `lumi-chapter-ingest/`). Per-skill symlinks `.claude/skills/lumi-*` point into `.agents/skills/lumi-<name>/`; this is the only place where the cross-platform symlink/junction/copy ladder still applies.
