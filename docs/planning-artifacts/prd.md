...
decisions:
  ...
  v01_skill_set: 14 skills total — core (6) `init/ingest/ask/edit/check/reset`; research pack (4) `discover/survey/prefill/setup`; reading pack (4) `chapter-ingest/character-track/theme-map/plot-recap`. MCP llm-review server and LaTeX/exp pipelines not bundled in v0.1 (no `novelty/review/refine/research/ideate/rebuttal/paper-*/exp-*/daily-arxiv`).
  ...
  v01_python_tools: 9 files in `_lumina/tools/` — `extract_pdf.py` (core, always installed; pypdf-based PDF text extractor used by `/lumi-ingest` and `/lumi-chapter-ingest`); research-pack adds `_env.py`, `discover.py`, `init_discovery.py`, `prepare_source.py`, plus four optional fetcher plugins (`fetch_arxiv.py`, `fetch_wikipedia.py`, `fetch_s2.py`, `fetch_deepxiv.py`)
inputDocuments:
...
- **`.agents/`** contains **only** `skills/`, laid out flat: one directory per skill named `lumi-<name>/` (e.g. `lumi-init/`, `lumi-research-discover/`, `lumi-reading-chapter-ingest/`). Per-skill symlinks `.claude/skills/lumi-*` point into `.agents/skills/lumi-<name>/`; this is the only place where the cross-platform symlink/junction/copy ladder still applies.
...
- Copy `core` skills into `.agents/skills/` as `lumi-<name>/` directories. Optionally install `research` and `reading` packs (also written as `lumi-<name>/` directories at the same level).
...
- Create per-skill symlinks `.claude/skills/lumi-*` → `.agents/skills/lumi-<name>/` when Claude Code is a selected target. This is the only place the symlink/junction/copy ladder applies.
...
- Bundled MCP server (`lumina_search`, `lumina_lint`, `lumina_ingest_url`) so any MCP-aware agent can call wiki operations natively. Note: v0.1 does **not** bundle an `llm-review` MCP server or second-provider config — shipping simplicity. Users who want second-model review can wire it in themselves.
...
**Rising action.** He runs `npx lumina-wiki@latest install` in the same project directory. The installer reads `_lumina/manifest.json`, recognizes this is an upgrade (not first install), and acts accordingly: it replaces files in `.agents/skills/` and `_lumina/schema/`, refreshes any symlink that has gone stale, leaves `wiki/` and `raw/` untouched, and rewrites `manifest.json` with the new version and skill set.
...
- **Claude Code** — entry points: `CLAUDE.md` at project root (rendered stub pointing to `README.md`), skill files at `.claude/skills/lumi-*/SKILL.md` (per-skill symlinks → `.agents/skills/lumi-<name>/`). Skills follow the SKILL.md + `references/` convention so the agent loads them on demand.
...
- **FR6:** Installer can copy the `core` skill set into `.agents/skills/` as `lumi-<name>/` directories on every install.
- **FR7:** Installer can optionally copy the `research` and `reading` packs into `.agents/skills/` as `lumi-<name>/` directories alongside the core skills, based on the User's pack selection.
...
- **FR8a:** Installer can copy Node engine scripts (`wiki.mjs`, `lint.mjs`, `reset.mjs`, `schemas.mjs`) into `_lumina/scripts/`, deeper schema reference docs (`page-templates.md`, `cross-reference-packs.md`, `graph-packs.md`) into `_lumina/schema/`, and the core PDF tool (`extract_pdf.py` + `requirements.txt`) into `_lumina/tools/` on every install. When the `research` pack is selected, Installer additionally copies the research-pack Python tools (`_env.py`, `discover.py`, `init_discovery.py`, `prepare_source.py`, fetchers) into the same `_lumina/tools/` directory. Pack-specific reference doc fragments are appended to `cross-reference-packs.md` and `graph-packs.md` only when the corresponding pack is installed.
...
- **FR10:** Installer can create per-skill symlinks under `.claude/skills/lumi-*` pointing into `.agents/skills/lumi-<name>/` for each installed skill when Claude Code is a selected IDE target. These per-skill symlinks are the only filesystem-link surface.
...
- **FR35:** Installer can ship the `research` pack containing **four** originally-authored skills covering the research-assistant workflow: `/lumi-research-discover` (ranked candidate shortlist), `/lumi-research-survey` (narrative synthesis from sources + concepts graph), `/lumi-research-prefill` (seed `foundations/` to prevent concept duplication on ingest), `/lumi-research-setup` (interactive API-key configuration; writes `<project>/.env`). Skill names are inspired by prior art; contents are written for Lumina-Wiki. v0.1 does **not** bundle cross-model review skills (`/novelty`, `/review`, `/refine`), the LaTeX paper pipeline (`/paper-plan`, `/paper-draft`, `/paper-compile`, `/rebuttal`), the experiment cluster (`/exp-design`, `/exp-run`, `/exp-status`, `/exp-eval`), the orchestrator (`/research`), `/ideate`, or `/daily-arxiv`. These are out of v0.1 scope — they depend on infrastructure (MCP llm-review server, remote GPU runner, LaTeX toolchain) that v0.1 does not commit to ship.
- **FR36:** Installer can ship the `reading` pack containing four skills: `/lumi-reading-chapter-ingest`, `/lumi-reading-character-track`, `/lumi-reading-theme-map`, `/lumi-reading-plot-recap`.
- **FR37:** Installer ships Python helper scripts into `_lumina/tools/`. The v0.1 set is: `extract_pdf.py` (always installed; pypdf-based PDF text extractor used by `/lumi-ingest` and `/lumi-chapter-ingest` when the host IDE cannot read PDFs natively — Codex, Gemini CLI, Cursor); plus, when the `research` pack is selected: `_env.py` (dotenv loader; reads `~/.env` then `<project>/.env`, project overrides global), `discover.py` (candidate ranking), `init_discovery.py` (multi-phase fetch with checkpoint manifest; writes fetched sources to `raw/discovered/`), `prepare_source.py` (normalize local PDF/tex into ingest-ready package; writes sidecars to `raw/tmp/`), and four optional fetcher plugins (`fetch_arxiv.py`, `fetch_wikipedia.py`, `fetch_s2.py`, `fetch_deepxiv.py`) — each fetcher activates only when the user provides its API key via `/lumi-research-setup`, and writes results into `raw/discovered/`. The Installer does not create a Python virtual environment in v0.1; pip-install of `requirements.txt` is deferred to the first invocation of a Python-needing skill (the file is copied alongside the tools so the user can `pip install -r _lumina/tools/requirements.txt`). v0.1 does not ship `remote.py` (no remote experiment runner) or output-side LaTeX preparation tools.

- **FR37a:** When the `research` pack is selected, the Installer can render `.env.example` at the project root listing every API key consumed by the shipped fetchers/tools (e.g. `SEMANTIC_SCHOLAR_API_KEY`, `DEEPXIV_TOKEN`) with one-line comments describing required vs optional status and where to obtain each key. The Installer never creates `.env` itself; `.env` is populated by the User directly or via `/lumi-research-setup`. `.env.example` is committed; `.env` is gitignored by the FR13 template.
